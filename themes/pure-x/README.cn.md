...
打开站点配置文件，找到theme字段，将其值更改为 pure-x
...
复制`theme/pure-x/_source/` 目录下`repository`文件夹到`blog path/source/` 目录下
...
复制`theme/pure-x/_source/` 目录下`books`文件夹到`blog path/source/` 目录下
...
复制`theme/pure-x/_source/` 目录下`links`文件夹到`blog path/source/` 目录下
